...
Statistics::Statistics()
{
	...
	p_oxygenBar->AddComponent(new Sprite("Placeholders/oxygenbar_placeholder"));
	p_oxygenBar->transform.SetPosition(uthEngine.GetWindow().GetViewport().getLeft() + 20, uthEngine.GetWindow().GetViewport().getTop() + 20);

	p_scoreText = new GameObject();
	p_scoreText->AddComponent(new Text("FreePixel", 50, "Score"));
	...
	p_gameTimeText->AddComponent(new Text("FreePixel", 50, "GameTime"));
...
{
	//transform.SetPosition(uthEngine.GetWindow().GetCamera().GetPosition());
	UpdateOxygenBar(dt);
	UpdateScore();
	UpdateGameTime(dt);
}
void Statistics::Draw(RenderTarget& target)
{
	p_gameTimeText->Draw(target);
	p_scoreText->Draw(target);
	p_oxygenBar->Draw(target);
}
